...
#define SCT_MIN_VOL			0
#define SCT_MAX_VOL			25
#define SCT_DEFAULT_VOL		SCT_MAX_VOL
//
...
#define XMP_SAMPLE_SYNTH	(1 << 15) /* Data contains synth patch */
	...
	/* SCT APIs for Keygen Creation */
	void SCT_Oynat(void* lpXM, void* param, int fwFlags);
	void SCT_XMduraklat(unsigned char state);
	void SCT_XMdurdur();

	void SCT_XMplay(void* lpXM, void* param, int fwFlags);
